...
async function ZerouConters(table) {
  const client = new MongoClient(uri);
  ...
  try {
    ...
    const database = client.db(dbName);
    const collection = database.collection(table);
    await collection.updateMany({}, { $set: { payment: 0, recipt: 0, invoice: 0, purcheis: 0, fixing: 0, Clients: 0 } })
    // db.counters.updateMany({}, { $set: { payment: 0, recipt: 0, invoice: 0, purcheis: 0, fixing: 0, Clients: 0 } })
  } finally {
    await client.close();
  }
}
